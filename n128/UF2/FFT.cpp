...
static complex<float> _buf[FFT_NUM];
...
void FFT_DIT_spatial_unroll_CY_stream_vector( 
   ...
   // #pragma HLS array_partition variable=data_6 type=cyclic factor=UF dim=1
    ...
    output_result_array_to_stream (data_5, dataOut);
   // FFT_stage_spatial_unroll<8>(data_5, data_6);
   // output_result_array_to_stream (data_6, dataOut);

}

void FFT_DFT_spatial_unroll_CY_stream_vector(
    hls::stream<hls::vector<complex<float>, UF*2>> & dataIn,  
    hls::stream<hls::vector<complex<float>, UF*2>> & dataOut
){
    // **Reverse-stage** decimation-in-frequency:
    // simply invoke the spatial-unroll stage function
    // in reverse order of stage index:
    #pragma HLS dataflow disable_start_propagation
    #pragma HLS performance target_ti=FFT_NUM/(2*UF) unit=cycle
    ...
    FFT_Stage1_vectorstream_parameterize (reverse_in_stream_vector, data_s1_stream_vector);
    FFT_Stage2_vectorstreamIn_arrayOut_parametize (data_s1_stream_vector, data_0);
    // Now run butterfly stages from last to first:
    // stage 7 … stage 3 in reverse:
    FFT_stage_spatial_unroll<7>(data_4, data_5);
    FFT_stage_spatial_unroll<6>(data_3, data_4);
    FFT_stage_spatial_unroll<5>(data_2, data_3);
    FFT_stage_spatial_unroll<4>(data_1, data_2);
    FFT_stage_spatial_unroll<3>(data_0, data_1);
    // now output:
    output_result_array_to_stream (data_1, dataOut);
}


void FFT_pipeline_DIT(hls::stream<hls::vector<complex<float>, UF*2>> & in, 
...
void FFT_pipeline_DIF(
  hls::stream<hls::vector<complex<float>,UF*2>>& in,
  hls::stream<hls::vector<complex<float>,UF*2>>& out,
  bool reverse_input,
  bool reverse_output
) {
  // same twiddle & revIdxTab initialization
  #pragma HLS dataflow disable_start_propagation
    LOOP_TWIDDLES: for (int i = 0; i < FFT_NUM/2; ++i) {
        double angle = -2 * PI * i / FFT_NUM;
        auto tw = complex<float>(cos(angle), sin(angle));
    ...
    //FFT_DFT_spatial_unroll_CY_stream_vector(in, out);

    // Stage 0: optional input reversal
    hls::stream<hls::vector<complex<float>,UF*2>> stage0;
    #pragma HLS stream variable=stage0 depth=8
    if (reverse_input) {
        for (int i = 0; i < FFT_NUM/UF; ++i) {
            auto v = in.read();
            for (int k = 0; k < UF*2; ++k)
                _buf[ revIdxTab[i*UF*2 + k] ] = v[k];
        }
        for (int i = 0; i < FFT_NUM/UF; ++i) {
            hls::vector<complex<float>,UF*2> outv;
            for (int k = 0; k < UF*2; ++k)
                outv[k] = _buf[i*UF*2 + k];
            stage0.write(outv);
        }
    } else {
        for (int i = 0; i < FFT_NUM/UF; ++i)
            stage0.write(in.read());
    }

    // Core DIF
    hls::stream<hls::vector<complex<float>,UF*2>> stage1;
    #pragma HLS stream variable=stage1 depth=8
    FFT_DFT_spatial_unroll_CY_stream_vector(stage0, stage1);

    // Stage 2: optional output reversal
    if (reverse_output) {
        for (int i = 0; i < FFT_NUM/UF; ++i) {
            auto v = stage1.read();
            for (int k = 0; k < UF*2; ++k)
        ...
        for (int i = 0; i < FFT_NUM/UF; ++i) {
            hls::vector<complex<float>,UF*2> outv;
            for (int k = 0; k < UF*2; ++k)
                outv[k] = _buf[ revIdxTab[i*UF*2 + k] ];
            out.write(outv);
        }
    } else {
        for (int i = 0; i < FFT_NUM/UF; ++i)
            out.write(stage1.read());
    }
...
static void bit_reverse_array() {
  for(int i=0; i<FFT_NUM; ++i) {
    int j = revIdxTab[i];
    if(j>i) std::swap(_buf[i], _buf[j]);
  }
}


// 1) DIT_RN: DIT-then-Reverse
// (run a DIT FFT on the natural-order input (which produces bit-reversed output), then reverse the output)
// <- easier to implement than Reverse-then-DIT
// in (bit-reverse) -> FFT_pipeline -> out (natural)
void FFT_DIT_RN(
  hls::stream<hls::vector<complex<float>,UF*2>>& in,
  hls::stream<hls::vector<complex<float>,UF*2>>& out
) {
  // local FIFO between core and buffer:
  hls::stream<hls::vector<complex<float>,UF*2>> mid;
#pragma HLS stream variable=mid depth=8

 // A) the core DIT pipeline into 'mid'
 FFT_pipeline_DIT(in, mid);
 // B) drain mid into a local array-of-vectors
  static hls::vector<complex<float>,UF*2> buffer[FFT_NUM/UF];  
  for (int i = 0; i < FFT_NUM/UF; i++) {
    buffer[i] = mid.read();
  }
// C) bit-reverse the entire buffer in place
  bit_reverse_array();  

  // D) stream the reveresed data out
  for (int i = 0; i < FFT_NUM/UF; i++) {
    out.write(buffer[i]);
  }
...
void FFT_DIF_NR(
    ...
    hls::stream<hls::vector<complex<float>,UF*2>>& out)
{
    FFT_pipeline_DIF(in, out, /*reverse_input=*/true, /*reverse_output=*/false);
}
...
void FFT_DIF_RN(
    ...
    hls::stream<hls::vector<complex<float>,UF*2>>& out)
{
    FFT_pipeline_DIF(in, out, /*reverse_input=*/false, /*reverse_output=*/true);
}
