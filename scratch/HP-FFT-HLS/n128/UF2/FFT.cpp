...
// simple on-chip buffer for bit-reversing
...
// 1) DIT_RN: Reverse-then-DIT, natural output
void FFT_DIT_RN(
  hls::stream<hls::vector<complex<float>,UF*2>>& in,
  hls::stream<hls::vector<complex<float>,UF*2>>& out
) {
  // create a local "pipeline" stream
  hls::stream<hls::vector<complex<float>,UF*2>> mid;
#pragma HLS stream variable=mid depth=8

 // read everything out of mid, bit-reverse in a buffer, then write into your top-level out
  static hls::vector<complex<float>,UF*2> buffer[FFT_NUM/UF];  
  for (int i = 0; i < FFT_NUM/UF; i++) {
    buffer[i] = mid.read();
  }
// run the bit-reverse of that array of vectors
  bit_reverse_array();  
  ...
  FFT_pipeline_DIT(mid, out);

}

// 2) DIF_NR: DIF-then-Reverse, bit-reversed output
void FFT_DIF_NR(
  hls::stream<hls::vector<complex<float>,UF*2>>& in,
  hls::stream<hls::vector<complex<float>,UF*2>>& out
) {
...
#pragma HLS stream variable=mid depth=8
  // 1) call pipeline_DIF
  FFT_pipeline_DIF(in, mid);

  // 2) read everything from in and bit-reverse into a buffer
  static hls::vector<complex<float>,UF*2> buffer[FFT_NUM/UF];
  for (int i = 0; i < FFT_NUM/UF; i++) {
    buffer[i] = in.read();
  }
  ...
  // 3) write the reversed data into mid
  for (int i = 0; i < FFT_NUM/UF; i++) {
    mid.write(buffer[i]);
  }
...
// 3) DIF_RN: Reverse-then-DIF, natural output
void FFT_DIF_RN(
  hls::stream<hls::vector<complex<float>,UF*2>>& in,
  hls::stream<hls::vector<complex<float>,UF*2>>& out
) {
...
#pragma HLS stream variable=mid depth=8
  ...
  // read everything from in and bit-reverse into a buffer
  static hls::vector<complex<float>,UF*2> buffer[FFT_NUM/UF];
  for (int i = 0; i < FFT_NUM/UF; i++) {
    buffer[i] = in.read();
  }
  ...
  // write the reversed data into mid
  for (int i = 0; i < FFT_NUM/UF; i++) {
    mid.write(buffer[i]);
  }

  FFT_pipeline_DIF(mid, out);
}
